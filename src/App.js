...
export default function App() {
  ...
  return (
    <div className="app-shell">
      ...
      {screen === 'domains'   && <DomainsScreen   navigate={navigate} scores={scores} />}
      {screen === 'game'      && <GameScreen      domain={activeDomain} onFinish={onGameFinish} navigate={navigate} />}
      {screen === 'result'    && <ResultScreen    result={lastResult} navigate={navigate} />}
      {screen === 'dashboard' && <DashboardScreen navigate={navigate} scores={scores} sessions={sessions} />}
    </div>
  );
}
